...
class Init {
	...
	public function __construct($token='',$rToken='') {
		global $config;
		if ($token != '') {
			$this->token = $token;
		}
	...
	public function set_home_dir($dir) { $this->home_dir = $dir; }
	...
	public function upload($filename,$mode='add',$location='') {
		$path = $filename;
	...
	public function create_shared_link($filename) {
		$data = array(
	...
	public function get_shared_link($filename) {
		$data = array('path' => $this->home_dir . $filename);
		$this->headers = array(
			'Authorization: Bearer '. $this->token,
			'Content-Type: application/json'
	...
	public function get_shared_link_file($url) {
		$data = array( 'url' => $url );
		$this->headers = array(
			'Authorization: Bearer '. $this->token,
			'Content-Type: application/json'
